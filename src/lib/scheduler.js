...
export function autoGenerate(year, month, members, leave, existingSched, lockedDays, holidayDays, rules, pairs, manualSchedule, useRandom) {
  ...
  if (nonDoctors.length > 0) {
    // Equal caps: distribute totalNonDoctorShifts as evenly as possible (difference ≤ 1).
    // useRandom shuffles who gets the +1 extra slot each regeneration.
  ...
  // Hard-cap filter: exclude members who have already reached their personal cap from
  // the primary pick pool. They stay in the fallback pool so slots are still filled
  // when everyone eligible has hit their cap (e.g. due to unavoidable leave coverage).
  const belowCap = m => !personalCap || (cnt[m.id] || 0) < (personalCap.get(m.id) ?? Infinity);
  ...
  for (let d = 1; d <= days; d++) {
    if (lockedDays && lockedDays.has(d)) {
      ...
      if (isWeekend(getDow(year, month, d))) {
        ...
        if (numRad === 0) {
          const pool = avail.filter(m => m.role === "radiologist" && !usedIds.has(m.id));
          pick(pool.filter(eligible).filter(belowCap), cnt, 1, pool.filter(eligible), sel, pairsMap, avoidMap, useRandom, personalCap).forEach(m => { result.push(m.id); usedIds.add(m.id); sel.add(m.id); });
        }
        if (numNurse === 0) {
          const pool = avail.filter(m => m.role === "nurse" && !usedIds.has(m.id));
          pick(pool.filter(eligible).filter(belowCap), cnt, 1, pool.filter(eligible), sel, pairsMap, avoidMap, useRandom, personalCap).forEach(m => { result.push(m.id); usedIds.add(m.id); sel.add(m.id); });
        ...
        if (needMore > 0) {
          ...
          pick(pool.filter(eligible).filter(belowCap), cnt, needMore, pool.filter(eligible), sel, pairsMap, avoidMap, useRandom, personalCap).forEach(m => { result.push(m.id); usedIds.add(m.id); sel.add(m.id); });
        }
      }
      sched[d] = result;
    ...
    if (isWeekend(dow)) {
      ...
      if (team.length === 0) {
        ...
        const sel = new Set();
        const result = [];
        const radPool = avail.filter(m => m.role === "radiologist");
        pick(radPool.filter(belowCap), cnt, r.weekend_radiologist, radPool, sel, pairsMap, avoidMap, useRandom, personalCap)
          .forEach(m => { result.push(m.id); sel.add(m.id); });
        const nursePool = avail.filter(m => m.role === "nurse" && !sel.has(m.id));
        pick(nursePool.filter(belowCap), cnt, r.weekend_nurse, avail.filter(m => m.role === "nurse"), sel, pairsMap, avoidMap, useRandom, personalCap)
      ...
      if (remaining > 0) {
        ...
        pick(rnPool.filter(eligible).filter(belowCap), cnt, remaining, rnPool.filter(eligible), sel, pairsMap, avoidMap, useRandom, personalCap, currentWeekCnt)
      ...
      const radAvail = avail.filter(m => m.role === "radiologist");
      pick(radAvail.filter(eligible).filter(belowCap), cnt, 1,
        radAvail.filter(eligible), sel, pairsMap, avoidMap, useRandom, personalCap, currentWeekCnt)
        .forEach(m => { result.push(m.id); used.add(m.id); sel.add(m.id); });
      const nurseAvail = avail.filter(m => m.role === "nurse" && !used.has(m.id));
      pick(nurseAvail.filter(eligible).filter(belowCap), cnt, 1,
        nurseAvail.filter(eligible), sel, pairsMap, avoidMap, useRandom, personalCap, currentWeekCnt)
        .forEach(m => { result.push(m.id); used.add(m.id); sel.add(m.id); });
      const filled = result.filter(id => { const role = getMember(id)?.role; return role === "radiologist" || role === "nurse"; }).length;
      const remaining = r.weekday_rad_nurse - filled;
      if (remaining > 0) {
